...
| `skills/genosdb/GOVERNANCE.md` | how a role is earned and who signs it: the constitution before the rules, the rules and how the engine evaluates them, who runs the engine, the application side, testing it |
...
This repository is the skill's only source; nothing here is generated or mirrored from elsewhere. It follows the engine's releases: when the public API changes, `API.md` changes with it. When the skill and the engine you run disagree, the CHANGELOG wins.
